...
func TestSubscriptionsCommands(t *testing.T) {
	...
	t.Run("/subscribe game-release adds entry to list", subscriptionsTestCase(func(t *testing.T) {
		ctx := NewMockContext(subscriptionsCE).WithGuild(channelIDs...).WithOptionString("type", hytale.GameFeedType.ID()).WithOptionChannel("channel", channelIDs[0]).WithOptionRole("role", roleID)
		...
		assert.Contains(t, extractContent(listCtx.replies[0]), hytale.GameFeedType.Display())
	}))

	t.Run("/subscribe maven-release adds DM subscription", subscriptionsTestCase(func(t *testing.T) {
		ctx := NewMockContext(subscriptionsCE).WithOptionString("type", hytale.MavenFeedType.ID())
		...
		assert.Contains(t, extractContent(listCtx.replies[0]), hytale.MavenFeedType.Display())
	}))
	...
	t.Run("/subscribe game-release and maven-release adds both to list", subscriptionsTestCase(func(t *testing.T) {
		...
		ctx1 := NewMockContext(subscriptionsCE).WithGuild(channelIDs...).WithOptionString("type", hytale.GameFeedType.ID()).WithOptionChannel("channel", channelIDs[0])
		...
		ctx2 := NewMockContext(subscriptionsCE).WithGuild(channelIDs...).WithOptionString("type", hytale.MavenFeedType.ID()).WithOptionChannel("channel", channelIDs[1])
	...
	t.Run("/subscribe twice for same channel updates version", subscriptionsTestCase(func(t *testing.T) {
		...
		ctx2 := NewMockContext(subscriptionsCE).WithGuild(channelIDs...).WithOptionString("type", hytale.GameFeedType.ID()).WithOptionChannel("channel", channelIDs[0])
		ctx2.RunCommand("subscribe")
	...
	t.Run("/unsubscribe without args clears all guild subscriptions", subscriptionsTestCase(func(t *testing.T) {
		...
		// Subscribe in DM
		ctx2 := NewMockContext(subscriptionsCE).WithOptionString("type", hytale.MavenFeedType.ID())
		ctx2.RunCommand("subscribe-dm")

		// Subscribe with role in channel2
		ctx3 := NewMockContext(subscriptionsCE).WithGuild(channelIDs...).WithOptionString("type", hytale.MavenFeedType.ID()).WithOptionChannel("channel", channelIDs[1]).WithOptionRole("role", roleID)
		...
		assert.Contains(t, content, hytale.GameFeedType.Display())
	...
	t.Run("/unsubscribe with channel removes from that channel only", subscriptionsTestCase(func(t *testing.T) {
		...
		assert.Contains(t, content, hytale.MavenFeedType.Display())
	}))
